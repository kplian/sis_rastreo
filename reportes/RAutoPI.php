...
class RAutoPI extends  ReportePDF {
	var $cabecera;
	var $detalleCbte;
	var $ancho_hoja;
	var $gerencia;
	...
	function Header() {
			    ...
			    ob_start();
				include(dirname(__FILE__).'/../reportes/tpl/auto_PI/cabecera.php');
	 ...
	 function generarReporte1() {
		...
		$with_col = $this->with_col;
         //adiciona glosa
		ob_start();
		include(dirname(__FILE__).'/../reportes/tpl/auto_PI/cuerpo.php');
        $content = ob_get_clean();
		
		ob_start();
		include(dirname(__FILE__).'/../reportes/tpl/auto_PI/detalle.php');
