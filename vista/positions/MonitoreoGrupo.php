...
//this.combo_segundos.setValue('10');
        ...
        showMap: function () {
            ...
            this.vectorSelectionLayer = new ol.layer.Vector({source: this.vectorSelection});
...
// generate resolutions and matrixIds arrays for this WMTS
            ...
            this.tileGrid = new ol.tilegrid.WMTS({
                origin: ol.extent.getTopLeft(projection1.getExtent()),
                resolutions: resolutions,
                matrixIds: matrixIds
            });
            this.layerCartografia = new ol.layer.Tile({
                title: 'Cartografia',
                type: 'base',
                source: new ol.source.WMTS({
                    ...
                    format: 'image/png',
                    url: 'http://192.168.30.97/api/webmap/elfec/wmts/webmap.cartography/{TileMatrixSet}/{TileMatrix}/{TileCol}/{TileRow}.png',
            ...
            this.baseGroup = new ol.layer.Group({
                title: 'Mapas Base',
                layers: [
                    this.layerOMSSatelite, this.layerOMS, this.layerCartografia
                ]
            });
            ...
            for (var i = 0; i < dataLayers.length; i++) {
                console.log(dataLayers[i]);
                overlayGroup.getLayers().push(new ol.layer.Tile({
                    title: dataLayers[i].name,
                    source: new ol.source.WMTS({
                        attributions: dataLayers[i].name,
                        url: dataLayers[i].url,
                        layer: dataLayers[i].layer,
                        matrixSet: dataLayers[i].matrixSet,
                        format: dataLayers[i].format,
                        tileGrid: this.tileGrid,
                        style: 'default',
                        wrapX: true,
                        requestEncoding: dataLayers[i].requestEncoding
                    }),
                    opacity: 1
                }));
            }
            this.map.addLayer(overlayGroup);
...
<b>Direccion:</b> {13}</br></br></div>";
        ...
        onDblclick: function (list, index, node, e) {
            var me = this;
            var record = list.getStore().getAt(index);
            if (record) {
                var features = record.get('geo_json');
                this.vectorSelection.clear();
                var config = {
                    latitud: features.geometry.coordinates[1],
                    longitud: features.geometry.coordinates[0],
                    course: 0,
                    data: {}
                };
                var aux = [parseFloat(config.longitud), parseFloat(config.latitud)];
                var point = ol.proj.fromLonLat(aux);
                var iconFeature = new ol.Feature(new ol.geom.Point(point));
                var iconStyle = new ol.style.Style({
                ...
                me.vectorSelection.addFeature(iconFeature);
                var extent = this.vectorSelection.getExtent();
                try {
                    this.map.getView().fit(extent, this.map.getSize());
                }
                catch (err) {
        ...
        showWindowsResult: function (data) {
            var me = this;
            ...
            var panelResumen = new Ext.Panel({
                padding: '0 0 0 20',
                html: '',
                split: true,
                layout: 'fit',
